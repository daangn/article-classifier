...
def cudnn_stack_bidirectional_dynamic_rnn(inputs, layer_sizes, sequence_length,
    ...
    if not is_training:
        ...
        if initial_state is None:
            ...
            zero_state = tf.zeros([batch_size, num_units])
            state_tuple = rnn_cell.LSTMStateTuple(zero_state, initial_state)
        ...
        last_states = tf.concat([output_state_fw[-1].h, output_state_bw[-1].h], 1)
    ...
    last_states = tf.concat([output_h[-2], output_h[-1]], 1)
    return outputs, last_states
